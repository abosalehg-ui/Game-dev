...
> ملف HTML واحد — Three.js مُضمَّن محلياً في `vendor/` فيعمل بالكامل دون إنترنت من أول زيارة.
...
npm test                         # اختبار دخان بالمتصفح (يقود دورة تطوير كاملة)
npm run icons                    # إعادة توليد أيقونات PWA (يتطلب Python + Pillow)
node tools/balance-sim.mjs       # محاكاة اقتصادية لموازنة التكلفة/الحصة
```
...
- **حالة الحفظ:** أي حقل يبدأ اسمه بـ `_` هو حالة جلسة مؤقتة **لا تُحفظ** إطلاقاً.
  عند تغيير مخطط الحفظ ارفع `SAVE_VERSION` وأضف الهجرة في `migrateSave`.
- **CI:** يعمل تلقائياً على كل PR (فحص الصياغة ← اختبار الدخان ← فحص إصدار الـ SW).
